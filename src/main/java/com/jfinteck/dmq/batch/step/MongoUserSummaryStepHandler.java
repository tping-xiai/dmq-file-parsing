package com.jfinteck.dmq.batch.step;
...
import com.jfinteck.dmq.batch.reader.MongodbDataItemReader;
import com.jfinteck.dmq.batch.writer.MySqlDataItemWriter;
...
//	@Autowired
//	private MySqlDataItemWriter<UserSummaryDTO> mySqlDataItemWriter;
	
	private String clazzName = "com.jfinteck.dmq.mongo.project.entity.UserSummary";
	private String statementId = "com.jfinteck.dmq.model.project.dao.UserSummaryDao.insert";
	
	private DecimalFormat form = new DecimalFormat("00.000");
	...
	public Step handleMongoDBStep() {
		...
		// 设置操作数据库表方法
		//mySqlDataItemWriter.setStatementId(statementId);
...
//				.processor(processor())
//				// 指定ItemWriter
//				.writer(mySqlDataItemWriter.writer())
//				.build();
		return null;
	}
	...
	/*
	 * @Bean public ItemProcessor<UserSummary, UserSummaryDTO> processor() { // TODO
	 * 对读取到的数据进行处理 return new ItemProcessor<UserSummary, UserSummaryDTO>() {
	 * 
	 * @Override public UserSummaryDTO process(UserSummary item) throws Exception {
	 * UserSummaryDTO userSummaryDTO = new UserSummaryDTO();
	 * item.setBeatRate(form.format(Double.valueOf(item.getBeatRate())));
	 * BeanUtils.copyProperties(userSummaryDTO, item); return userSummaryDTO; } }; }
	 */
